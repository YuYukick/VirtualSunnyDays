...
cap = cv2.VideoCapture(0) # CAMERA asign
...
try:
	...
	date_time_1 = datetime(now.year, now.month, now.day,now.hour,now.minute+GET_DATA_CYCLE,now.second)
	...
	while True:
		...
		if now == date_time_1:
			cv2.imwrite('./sousei_hukuda/result_'+str(now.month)+str(now.day)+"_"+str(now.hour)+str(now.minute)+"_"+str(s_time)+'.png',org_img)
		...
		if key == 27: #pushing ESC key, then finish
			...
			### save images ###
			cv2.imwrite('./sousei_hukuda/org_'+str(s_time)+'.png',org_img)
			cv2.imwrite('./sousei_hukuda/processed_'+str(s_time)+'.png',final_img)
